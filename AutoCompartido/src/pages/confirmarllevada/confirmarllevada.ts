   ...
   // this.servicio.definirSolicitarRef(this.rama[0],this.parser[0],this.data.fecha,this.data.hora);
    ...
    /*this.servicio.addSolicitud(this.solicitud).then(
      ref=>{
       ...
       // this.mostrarAlerta();
        //pagina de control
        this.navCtrl.setRoot(VerRutaDesdePasajeroPage,{email:this.email});
        
      }
    );*/
      this.distancia(this.data);
      this.navCtrl.setRoot(VerRutaDesdePasajeroPage,{email:this.email,otro:this.data.email,latitud:this.latitud,longitud:this.longitud});
  ...
  distancia(data){
    ...
    for(let i=0;i<puntos.length;i++){
      ...
      if(data.pasajeros==null){
        ...
        data.pasajeros=data.pasajeros+';'+this.email;
      }
